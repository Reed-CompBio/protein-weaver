---
title: "Zebrafish Data Processing and Merging"
...
psicquic_interactome <- psicquic_interactome %>%
  group_by(uniprotID1, uniprotID2, source) %>%
  slice_min(order_by = link, with_ties = FALSE) %>%
  ungroup()
```

Save the PsicQuic data to a separate file
```{r}
write_csv(psicquic_interactome, "./zfish_psicquic_results_2024-07-30.csv")
```
...
colnames(string_db_interactome) <- c("stringdbID1", "stringdbID2", "uniprotID1", "uniprotID2")

# merge both columns to help with string db URL
...
string_db_interactome <- 
  string_db_interactome %>% select("uniprotID1", "uniprotID2", "link") %>% 
  mutate(source = "string-db")
```

Now, merge the PsicQuic and String-DB interactomes together.
```{r}
psicquic_interactome <- psicquic_interactome %>% select(uniprotID1, uniprotID2, link, source)
...
colnames(zfish_interactome) <- c("uniprotID1", "uniprotID2", "link",  "source","alt_name1", "alt_name2")
...
```{r}
write_tsv(zfish_interactome, "./interactome_txid7955_2024-07-30.txt")
```
